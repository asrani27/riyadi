...
<div class="row">
    <div class="col-md-12">
        <div class="box box-primary">
            ...
            <!-- /.box-header -->
            <!-- form start -->
            <form role="form" method="post" action="/t/pkg/edit/{{$data->id}}">
                @csrf
                <div class="box-body">
                    <div class="form-group">
                        ...
                        <select name="periode_id" class="form-control" required>
                            ...
                            <option value="{{$item->id}}" {{$data->periode_id == $item->id ?
                                'selected':''}}>{{$item->tahun}} - {{$item->semester == '1' ?
                                'ganjil':'genap'}}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="guru_id" class="form-control" required>
                            ...
                            <option value="{{$item->id}}" {{$data->guru_id == $item->id ?
                                'selected':''}}>{{$item->nama}}</option>
                            @endforeach
                        </select>
                    </div>
